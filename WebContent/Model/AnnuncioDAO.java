	...
	public int cancellaRiferimenti(String targa) throws SQLException 
	{
		...
		try {
			...
			while (rs.next()) 
			{
				...
				// ottiene l'annuncio per ricavarne il prezzo
				AnnuncioDAO annunciodao = new AnnuncioDAO();
				AnnuncioBean annuncio = annunciodao.doRetrieveByKey(targa);
				
				// modifica il totale del carrello
				carrello.setTotale(carrello.getTotale() - annuncio.getPrezzo());
				
				// salva nel db il nuovo carrello aggiornato
				carrellodao.doDelete(carrello.getCodice_carrello());
